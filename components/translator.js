...
class Translator {
    ...
    let otherBritish = Object.values(americanToBritishTitles).concat(Object.values(americanToBritishSpelling));
    ...
    for (const [key, value] of Object.entries(americanToBritishSpelling)) {
        const regBrit = new RegExp(`\\b${value}\\b`, "ig");
        if (regBrit.test(text)) {            
            text = text.replace(regBrit, `<span class=highlight>${key}</span>`);
            i++;
        }
    }
